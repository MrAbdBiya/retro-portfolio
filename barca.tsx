...
function BarcaShowcase() {
  ...
  return (
    <div className="min-h-screen blaugrana-bg">
      {/* Hero */}
      ...
      <main className="relative -mt-24 pb-24">
        <section className="max-w-6xl mx-auto px-6 grid md:grid-cols-3 gap-6">
          {[
            ...
            <div key={i} className="glass rounded-2xl p-6 shine">
        ...
        <section id="squad" className="max-w-6xl mx-auto px-6 mt-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {['Ter Stegen','Araujo','Gündoğan','Pedri','De Jong','Yamal','Lewandowski','Raphinha'].map((p) => (
              <div key={p} className="aspect-[4/5] rounded-xl glass overflow-hidden flex items-end p-4 shine">
                <div>
                  <div className="text-sm text-white/70">#{Math.floor(Math.random()*99).toString().padStart(2,'0')}</div>
        ...
        <section id="kits" className="max-w-6xl mx-auto px-6 mt-20">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[{n:'Domicile', c:'from-[var(--barca-blue)] to-[var(--barca-grana)]'}, {n:'Extérieur', c:'from-[#0d0d0d] to-[#222]'}, {n:'Third', c:'from-[#1B3B6F] to-[#FDB927]'}].map((k) => (
              <div key={k.n} className="rounded-2xl p-6 glass">
